...
bjam.exe -sPYTHON_SOURCE=%LUX_X86_PYTHON2_ROOT% --toolset=msvc-9.0  --with-python --stagedir=stage/python --build-dir=bin/python2 python=2.6 target-os=windows stage

rem echo.
rem echo **************************************************************************
rem echo * Building Boost::Python3                                                *
rem echo **************************************************************************
rem copy /Y %LUX_X86_PYTHON3_ROOT%\PC\pyconfig.h %LUX_X86_PYTHON3_ROOT%\Include
rem copy /Y %BUILD_PATH%\support\x86-project-config-31.jam .\project-config.jam
rem bjam.exe -sPYTHON_SOURCE=%LUX_X86_PYTHON3_ROOT% --toolset=msvc-9.0  --with-python --stagedir=stage/python3 --build-dir=bin/python3 python=3.1 target-os=windows stage
...
msbuild /verbosity:minimal /property:"Configuration=Release;Platform=Win32" /property:"VCBuildOverride=%BUILD_PATH%\support\LuxFreeImage.vsprops" /target:"FreeImageLib" FreeImage.2008.sln
