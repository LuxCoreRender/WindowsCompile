...
SET BJAM_OPTS=-a -q -j%NUMBER_OF_PROCESSORS% address-model=64 link=static threading=multi runtime-link=shared --with-date_time --with-filesystem --with-iostreams --with-program_options --with-python --with-regex --with-serialization --with-system --with-thread --stagedir=stage/boost --build-dir=bin/boost -sBZIP2_SOURCE=%LUX_X64_BZIP_ROOT% -sPYTHON_SOURCE=%LUX_X64_PYTHON3_ROOT% -sZLIB_SOURCE=%LUX_X64_FREEIMAGE_ROOT%\FreeImage\Source\ZLib
...
IF %BUILD_DEBUG% EQU 1 ( msbuild /m /verbosity:minimal /property:"Configuration=Debug" /property:"Platform=x64" /target:"Clean" /target:"FreeImageLib" FreeImage.2010.sln )
msbuild /m /verbosity:minimal /property:"Configuration=Release" /property:"Platform=x64" /target:"Clean" /target:"FreeImageLib" FreeImage.2010.sln
...
IF %BUILD_DEBUG% EQU 1 ( msbuild /m /verbosity:minimal /property:"Configuration=Debug_Static" /property:"Platform=x64" /target:"Clean" /target:"freeglut" VisualStudio\2010\freeglut.sln )
msbuild /m /verbosity:minimal /property:"Configuration=Release_Static" /property:"Platform=x64" /target:"Clean" /target:"freeglut" VisualStudio\2010\freeglut.sln
...
IF %BUILD_DEBUG% EQU 1 ( msbuild /m /verbosity:minimal /property:"Configuration=Debug" /property:"Platform=x64" /target:"Clean" /target:"glew_static" build\vc10\glew.sln )
